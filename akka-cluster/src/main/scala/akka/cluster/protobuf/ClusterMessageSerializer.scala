...
class ClusterMessageSerializer(val system: ExtendedActorSystem) extends Serializer {
  ...
  private def addressToProto(address: Address): msg.Address =
    msg.Address(
      address.system,
      if (address.host.isDefined) address.host.get else "",
      if (address.port.isDefined) address.port.get else 0,
      Some(address.protocol))
  ...
  private def addressFromProto(address: msg.Address): Address =
    Address(
      if (address.protocol.isDefined) address.protocol.get else "",
      address.system,
      address.hostname,
      address.port)
  ...
  private def gossipToProto(gossip: Gossip): msg.Gossip = {
    ...
    val allHashes = gossip.overview.seen.values.foldLeft(gossip.version.versions.keys.toSet)({ case (s, VectorClock(v)) ⇒ s ++ v.keys }).to[Vector]
